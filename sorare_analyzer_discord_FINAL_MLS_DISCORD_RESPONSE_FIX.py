...
async def analyse(
    ...
    # Discord muss innerhalb weniger Sekunden eine echte Antwort bekommen.
    # Deshalb senden wir die Startmeldung DIREKT, bevor irgendeine Analyse
    # oder Sorare-Abfrage beginnt. Die lange Analyse läuft danach im Thread.
    discord_user_id = interaction.user.id
    channel = interaction.channel

    if discord_user_id not in DISCORD_SORARE_ACCOUNTS:
        await interaction.response.send_message(
            "❌ Für deine Discord-ID ist noch kein Sorare-Account hinterlegt.",
            ephemeral=True,
        )
        return
...
async def on_ready():
    ...
    print("Version: FINAL + MLS Saison 2026 + sofortige Discord-Antwort")
