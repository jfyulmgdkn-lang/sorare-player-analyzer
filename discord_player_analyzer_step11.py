...
def calculate_analyzer(scores):
    ...
    availability = clamp(len(clean) / EXPECTED_GAMES * 100.0)
    consistency = clamp(100.0 - stddev * 2.0)
...
def fetch_player_scores(api_key, slug):
    ...
    stats = calculate_analyzer(player.get("rawPlayerGameScores") or [])
...
def run_analysis(api_key, league_name, position_name, budget_name):
    ...
    results.sort(
        key=lambda item: item["stats"]["score"],
        reverse=True,
    )
...
def create_embed(results, league_name, position_name, budget_name):
    embed = discord.Embed(
        title=f"🏆 Top 20 – {league_name} – {position_name}",
        description=(
    ...
    for index, player in enumerate(results, start=1):
        ...
        lines.append(
            f"**{index}. {player['name']}** — {s['score']:.2f}/100\n"
            f"L5 {s['l5']:.1f} | L10 {s['l10']:.1f} | L15 {s['l15']:.1f} | "
    ...
    embed.set_footer(
        text="Analyzer: 40% Form | 20% Konstanz | 15% 60+ | 10% 75+ | 5% 90+ | 10% Verfügbarkeit"
    )
